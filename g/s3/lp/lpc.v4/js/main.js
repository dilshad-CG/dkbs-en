;(function () {
  var $win = $(window),
	...
	lpc_template.queue.lpcQueryProduct = function ($self) {
		...
		if ($blocks.length) {
			...
			if(!s3LP.is_cms){
				$(document).keyFilter('.lpc-product-amount input:text', {
  ...
  lpc_template.queue.formInputs = function ($self) {
	...
	lpc_template.queue.anchorMenu = function ($self) {
		...
		let button = $block.find('.lpc-anchor-menu-fixed__button');
		...
		/*if (isMobile) {
		    $(document)
		        .off('click.anchorMenu')
		        .on('click.anchorMenu', '.lpc-anchor-menu-fixed__button', function (e) {
		            e.preventDefault();
		            e.stopPropagation();
		
		            $(this)
		                .closest('.lpc-anchor-menu-fixed')
		                .toggleClass('active');
		        });
		
		    $(document).on('keydown.anchorMenu', function (e) {
		        if (e.keyCode === 27) {
		            $('.lpc-anchor-menu-fixed').removeClass('active');
		        }
		    });
		
		    $(document).on('click.anchorMenuClose', function (e) {
		...
		if (isMobile) {
			$(document).off('click.anchorMenu').on('click.anchorMenu','.lpc-anchor-menu-fixed__button', function (event) {
					event.preventDefault();
					event.stopPropagation();
					$(this).closest('.lpc-anchor-menu-fixed').toggleClass('active');
				}
			);
		
			$(document).off('keydown.anchorMenu').on('keydown.anchorMenu', function (event) {
				if (event.key === 'Escape') {
					$('.lpc-anchor-menu-fixed').removeClass('active');
				}
			});
		
			$(document).off('click.anchorMenuClose').on('click.anchorMenuClose', function (event) {
				if (!$(event.target).closest('.lpc-anchor-menu-fixed').length) {
					$('.lpc-anchor-menu-fixed').removeClass('active');
				}
			});
		}
		...
		links.each(function () {
			...
			$this.off('click.anchorMenuLink').on('click.anchorMenuLink', function (event) {
				event.preventDefault();
		
				if (!targetBlock.length || !targetBlock[0]) return;
				...
				const animateScroll = function (currentTime) {
					...
					/*
					 * Позиция пересчитывается на каждом кадре.
					 * Поэтому изменение высоты страницы не вызывает скачок.
					 */
					const currentTargetPosition = getTargetPosition();
	...
	lpc_template.queue.ImageParallax = function ($self) {

		let $blocks = $self.find('.init-image-parallax');
		if (!$blocks.length) return;
	
		$blocks.each(function () {
	
			const $block = $(this);
			const image = $block.find('.lpc-promotions-new__image')[0];
			if (!image || isMobile) return;
			
			const promotion = $block.closest('.lpc-promotions-new')[0];
			if (!promotion) return;
			...
			promotion.addEventListener('mousemove', (e) => {
				const rect = promotion.getBoundingClientRect();
				const x = e.clientX - rect.left;
				const y = e.clientY - rect.top;
	
				const offsetX = (x / rect.width - 0.1) * 1;
				const offsetY = (y / rect.height - 0.1) * 1;
			...
			promotion.addEventListener('mouseleave', () => {
				targetX = 0;
				targetY = 0;
				if (!frame) frame = requestAnimationFrame(animate);
			});
	...
	lpc_template.queue.aroundMagic = function ($self) {
		...
		const items = $animation.find('.lpc-recalculation-title');
	...
	lpc_template.queue.stepArrowPositioning = function ($self) {

		const $blocks = $self.find('.init-arrow-positioning');
		if (!$blocks.length) return;

		function updateStepsArrows($block) {

			const mediaSource = $('.decor-wrap').attr('data-media-source');
			const $arrows = $block.find('.lpc-steps-new__arrow');
		
			if (!['media-xl', 'media-lg', 'media-md'].includes(mediaSource)) {
				$arrows.css({
					top: '',
					marginLeft: ''
				});
			...
			const $items = $block.find('.lpc-steps-new__item');

			$items.each(function (index) {

				const $item  = $(this);
				const $arrow = $item.find('.lpc-steps-new__arrow');
				const $icon  = $item.find('.lpc-steps-new__icon-box');

				if (!$arrow.length || !$icon.length) return;
				...
				const top = (iconRect.top - itemRect.top) + (iconRect.height / 2) - (arrowRect.height / 2);

				const centerBetween = (itemRect.right + nextItemRect.left) / 2;

				const marginLeft = centerBetween - itemRect.left - (arrowRect.width / 2);
		...
		$blocks.each(function () {
			...
			const run = () => updateStepsArrows($block);
	...
	lpc_template.queue.tabsConditions = function ($self) {
		...
		$block.each(function () {
			...
			let isGroup = $head.hasClass('_group_active');
	
			if ($head.hasClass('_type-group')) {
				$head.css('border-radius', $lpButton.css('border-radius'));
			}
			...
			function syncButtonTypes() {
				$buttons
					.removeClass('lpc-button--type-1 lpc-button--type-2')
					.addClass('lpc-button--type-2');
	
				$buttons.filter('.active')
					.removeClass('lpc-button--type-2')
					.addClass('lpc-button--type-1');
			}
			...
			$buttons.on('click', function () {
				...
				$items.each(function () {
					let $item = $(this);
					$item.toggleClass('active', $item.data('index') == index && !isActive);
				});
	...
	document.addEventListener("lpc_init_after", function () {
		// Функция для обновления window.s3LP за пределами айфрейма 
		// https://staff.megagroup.ru/staff/sites/?site_id=8861349 используется в файле /g/s3/lp/lpc_tpl/js/lpc_tpl_main.js
		// для того чтобы проверять из внешнего файла находимся мы в конструкторе или нет и что-то делать
		function syncS3LPToTop() {
		    try {
		        if (window.top && window.top !== window && window.s3LP) {
		            window.top.s3LP = window.s3LP;
		        }
		    } catch (e) {
		        console.warn('Нет доступа к window.top', e);
		    }
		}
	...
	lpc_template.queue.sliderBlock = function ($self) {
		...
		if ($block.data('slider-thumb-init') != true) {
	        $block.each(function(){
	            ...
	            if($this.find('.splide').not('.is-active').length != 0) {
	            	let $splideElem = $this.find('.splide').not('.is-active');
					let slidesCount = $splideElem.find('.splide__slide').length;
					
					function getSliderBehaviorConfig() {
						let currentPerPage = checkInitPerPage();
						let isDragAvailable = slidesCount > currentPerPage;
					
						return {
							perPage: currentPerPage,
							drag: isDragAvailable && !$this.data('drag'),
							clones: isDragAvailable ? undefined : 0,
							pagination: isDragAvailable
						};
					}
					
	                let splide = new Splide($splideElem[0], $.extend({
						autoplay: $this.data('autoplay'),
						speed: $this.data('speed'),
						interval: $this.data('pause'),
	                    rewind: $this.data('infinite'),
						...
						noDrag: '.no-drag',	
						perPage: checkInitPerPage()
	                }, getSliderBehaviorConfig()));
					
	                splide.mount();
					sliderBreakPoints();
					
					function SafeGliderUpdate() {
						splide.options = getSliderBehaviorConfig();
						splide.refresh();
					}
					
					document.addEventListener('lpcPopupOpened',  SafeGliderUpdate);
					document.addEventListener('dataMediaSourceChange', function() {
						sliderBreakPoints();
						SafeGliderUpdate();
					});
